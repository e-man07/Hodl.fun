...
// Explicit spacing variants instead of boolean props (patterns-explicit-variants rule)
type DropdownMenuSpacing = 'default' | 'inset';

const DROPDOWN_MENU_SPACING = {
  default: '',
  inset: 'pl-8',
} as const;
...
function DropdownMenuSubTrigger({ className, spacing = 'default', children, ref, ...props }: DropdownMenuSubTriggerProps) {
  return (
    <DropdownMenuPrimitive.SubTrigger
      ref={ref}
      className={cn(
        'flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none focus:bg-accent data-[state=open]:bg-accent',
        DROPDOWN_MENU_SPACING[spacing],
        className
      )}
      {...props}
    >
...
function DropdownMenuItem({ className, spacing = 'default', ref, ...props }: DropdownMenuItemProps) {
  return (
...
function DropdownMenuLabel({ className, spacing = 'default', ref, ...props }: DropdownMenuLabelProps) {
  return (
    <DropdownMenuPrimitive.Label
      ref={ref}
      className={cn('px-2 py-1.5 text-sm font-semibold', DROPDOWN_MENU_SPACING[spacing], className)}
      {...props}
    />
  );
}
